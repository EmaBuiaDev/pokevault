...
object AppLocale {
    ...
    val matchLocation: String get() = if (isItalian) "Luogo" else "Location"
    val matchLocationPlaceholder: String get() = if (isItalian) "Es. Game Store Milano, League Cup..." else "E.g. Game Store Milan, League Cup..."
    val matchDate: String get() = if (isItalian) "Data" else "Date"
    val matchFormat: String get() = if (isItalian) "Formato" else "Format"
    val matchDeckUsed: String get() = if (isItalian) "Mazzo usato" else "Deck used"
    val matchDeckName: String get() = if (isItalian) "Nome mazzo" else "Deck name"
    val matchDeckList: String get() = if (isItalian) "Lista mazzo (opzionale)" else "Deck list (optional)"
    ...
    val matchLogEmptySubtitle: String get() = if (isItalian) "Registra la tua prima partita competitiva!" else "Log your first competitive match!"
