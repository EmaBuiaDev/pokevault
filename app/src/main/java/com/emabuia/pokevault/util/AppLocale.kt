...
object AppLocale {
    ...
    val organizeCardsHint: String get() = if (isItalian) "Organizza le carte senza perdere il contesto" else "Organize cards without losing context"
    val resetFilters: String get() = if (isItalian) "Reset" else "Reset"
    val showResults: String get() = if (isItalian) "Mostra risultati" else "Show results"
    val expansionOrder: String get() = if (isItalian) "Ordine espansioni" else "Expansion order"
    val moreCards: String get() = if (isItalian) "Piu carte" else "More cards"
    val fewerCards: String get() = if (isItalian) "Meno carte" else "Fewer cards"
    val expandAll: String get() = if (isItalian) "Espandi tutte" else "Expand all"
    val collapseAll: String get() = if (isItalian) "Chiudi tutte" else "Collapse all"
    fun selectedCount(count: Int): String = if (isItalian) "$count selezionate" else "$count selected"

    val noCardFound: String get() = if (isItalian) "Carta non trovata" else "Card not found"
    val gradedCardSection: String get() = if (isItalian) "Carta Gradata" else "Graded card"
    val insertInGradedCards: String get() = if (isItalian) "Inserisci nelle carte gradate" else "Add to graded cards"
    val gradeLabel: String get() = if (isItalian) "Voto (1-10)" else "Grade (1-10)"
    val gradingAgency: String get() = if (isItalian) "Ente" else "Agency"
    val details: String get() = if (isItalian) "Dettagli" else "Details"
    val estimatedValue: String get() = if (isItalian) "Valore stimato" else "Estimated value"
    val livePrices: String get() = if (isItalian) "Prezzi Live" else "Live prices"
    val loadingPrices: String get() = if (isItalian) "Caricamento prezzi..." else "Loading prices..."
    val cardMarket: String get() = "CardMarket"
    val openOnCardMarket: String get() = if (isItalian) "Apri su CardMarket" else "Open on CardMarket"
    val averagePrice: String get() = if (isItalian) "Prezzo medio" else "Average price"
    val trend: String get() = if (isItalian) "Trend" else "Trend"
    ...
    val goBack: String get() = if (isItalian) "Torna indietro" else "Go back"
    val recognizedCard: String get() = if (isItalian) "Carta riconosciuta" else "Recognized card"
    val add: String get() = if (isItalian) "Aggiungi" else "Add"
    val myVariantsLabel: String get() = if (isItalian) "Varianti in tuo possesso:" else "Owned variants:"
    fun playersCount(count: Int): String = if (isItalian) "$count giocatori" else "$count players"

    val createNewDeck: String get() = if (isItalian) "Crea un nuovo deck" else "Create a new deck"
    val importDeck: String get() = if (isItalian) "Importa Deck" else "Import deck"
    val importInDeckLab: String get() = if (isItalian) "Importa in DeckLab" else "Import into DeckLab"
    val deckNamePlaceholder: String get() = if (isItalian) "Nome deck..." else "Deck name..."
    val clearSearch: String get() = if (isItalian) "Cancella ricerca" else "Clear search"
    val noCardsInCollectionMatch: String get() = if (isItalian) "Nessuna carta trovata nella tua collezione." else "No cards found in your collection."
    val searchInTcgSets: String get() = if (isItalian) "Cerca nei set TCG" else "Search in TCG sets"
    val noCardsInCategory: String get() = if (isItalian) "Nessuna carta in questa categoria.\nAggiungi carte alla tua collezione o cerca nei set." else "No cards in this category.\nAdd cards to your collection or search in sets."
    val import: String get() = if (isItalian) "Importa" else "Import"
    val choose2Covers: String get() = if (isItalian) "Scegli 2 Copertine" else "Choose 2 covers"
    val close: String get() = if (isItalian) "Chiudi" else "Close"
    val ok: String get() = "OK"
    val refresh: String get() = if (isItalian) "Aggiorna" else "Refresh"
    val connectionError: String get() = if (isItalian) "Errore di connessione" else "Connection error"
    val noMetaDeckFound: String get() = if (isItalian) "Nessun meta deck trovato" else "No meta deck found"
    val tryChangeFormat: String get() = if (isItalian) "Prova a cambiare formato o riprova piu tardi." else "Try changing format or retry later."
    val viewDeck: String get() = if (isItalian) "Vedi deck" else "View deck"
    val noDecklistAvailable: String get() = if (isItalian) "Nessuna decklist disponibile" else "No decklist available"
    fun deckCountLabel(count: Int): String = if (isItalian) "$count mazzi" else "$count decks"
    val unknownDeck: String get() = if (isItalian) "Deck sconosciuto" else "Unknown deck"

    val minus: String get() = if (isItalian) "Meno" else "Minus"
    val plus: String get() = if (isItalian) "Piu" else "Plus"
    val howManyCopiesToAdd: String get() = if (isItalian) "Quante copie vuoi aggiungere?" else "How many copies do you want to add?"
    val addedToCollectionAndDeck: String get() = if (isItalian) "Verranno aggiunte alla tua collezione e al deck" else "They will be added to your collection and deck"
    fun addCopiesToDeck(count: Int): String = if (isItalian) "Aggiungi $count al deck" else "Add $count to deck"
    ...
    val setExpansionLabel: String get() = if (isItalian) "Set / Espansione" else "Set / Expansion"
    val cardNamePlaceholder: String get() = if (isItalian) "es. Charizard VMAX" else "e.g. Charizard VMAX"
    val setPlaceholder: String get() = if (isItalian) "es. Base Set, Evolving Skies" else "e.g. Base Set, Evolving Skies"
    val hpPlaceholder: String get() = if (isItalian) "es. 180" else "e.g. 180"
    val valueLabel: String get() = if (isItalian) "Valore (euro)" else "Value (EUR)"
    val valuePlaceholder: String get() = if (isItalian) "es. 25.50" else "e.g. 25.50"
    val gradedHint: String get() = if (isItalian) "PSA, BGS, CGC" else "PSA, BGS, CGC"
    val gradeFieldLabel: String get() = if (isItalian) "Voto (Grade)" else "Grade"
    val gradePlaceholder: String get() = if (isItalian) "es. 9.5" else "e.g. 9.5"
    val additionalNotesPlaceholder: String get() = if (isItalian) "Note aggiuntive..." else "Additional notes..."
    val imageUrlOptionalLabel: String get() = if (isItalian) "URL immagine (opzionale)" else "Image URL (optional)"
    ...
    val albumSizeCustom: String get() = if (isItalian) "Personalizzato" else "Custom"
    val albumCustomSizeLabel: String get() = if (isItalian) "Numero carte personalizzato" else "Custom card count"
    ...
    val albumSearchInCollection: String get() = if (isItalian) "Cerca nella tua collezione" else "Search your collection"
    val albumSearchPlaceholder: String get() = if (isItalian) "Nome, set o numero carta" else "Name, set or card number"
