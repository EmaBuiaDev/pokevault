...
| 5 | ~~Set giapponesi taggati erroneamente come ENG~~ | `viewmodel/SetsViewModel.kt:118` — **chiusa 2026-09-09**: riferimento ormai inesistente (file riscritto), e il meccanismo originale non si applica piu'. Vedi checkpoint in fondo |
...
| 15 | Spezzare i file monolitici (`DeckLabScreen.kt` 2240 righe, `PokeTcgRepository.kt` 1735) | — **RIAPERTA**: lo split del 2026-09-09 (2241 -> 6 file) e' stato **annullato dal merge** dello stesso giorno, perche' operava sulla versione del file precedente alla riscrittura DeckLab di R3.0.0 e ne avrebbe duplicato ogni simbolo. `DeckLabScreen.kt` e' oggi 2291 righe: se si rifa', va rifatto sulla versione attuale. `CatalogRepository.kt` (ex `PokeTcgRepository.kt`) non e' lo stesso tipo di lavoro, vedi checkpoint |
...
### Bug segnalato dall'utente: carte Energia "senza immagine" -- una causa trovata e corretta, non confermata come LA causa

`deriveItalianSupertype()` leggeva `record.tipo` (il tipo elementale, Fuoco/Acqua/...) per riconoscere Trainer/Energy -- ma quel campo e' popolato **solo** per le carte Pokemon (confermato via D1: nessuna carta con nome `Energia %` ha mai `ps` o `tipo` valorizzati). Le due condizioni non scattavano mai: ogni carta Energia cadeva sul ramo else e veniva classificata **"Trainer"**, mai "Energy" -- impatta il conteggio "Energy" in Analisi Lab e ovunque si legga `card.supertype` direttamente. Corretto: HP>0 -> Pokemon, altrimenti nome che inizia per "Energia" -> Energy (verificato sull'elenco completo dei nomi distinti in D1 per escludere falsi positivi tipo "Recupero di Energia Plus", un Trainer), altrimenti Trainer.

**Non confermato pero' che questo sia il bug delle immagini mancanti**: prima di scrivere il fix, testate dal vivo 34 carte Energia campione (30 casuali su tutto il catalogo + 4 mirate su epoche diverse, da `base`/`xy` a `sv`) contro l'URL che l'app costruisce davvero -- **34/34 rispondono 200** via Worker/R2, byte reali. La pipeline di risoluzione immagine non e' rotta lato server/dati. Se dopo questo fix e una nuova build le immagini restano mancanti, serve un esempio concreto (schermata + nome carta specifico) per continuare l'indagine lato client (es. percorso Scanner/OCR, che non e' stato ancora controllato).

### Sez. 8 #5 (set giapponesi taggati come ENG) -- chiusa, meccanismo originale non piu' applicabile

Il riferimento del piano (`SetsViewModel.kt:118`) non esiste piu' -- il file e' cambiato radicalmente con le riscritture del Pokedex. Verificato il meccanismo attuale: il Pokedex mostra oggi **solo** le espansioni con `language == "ITA"` (`SetsViewModel.applyFilters()`, valore impostato a mano in `buildItalianTcgSet`/`mergeItalianSets`, mai letto dal campo lingua grezzo di PokeWallet) -- un set PokeWallet etichettato male non arriva piu' direttamente all'utente come nella vecchia architettura pre-ITA-only.

**Caso limite residuo, non un bug aperto**: se PokeWallet etichetta un set giapponese-esclusivo come "English", `langPriority()` in `mergeItalianSets()` lo tratterebbe comunque come match ENG valido per il collegamento nome/logo/serie di un'espansione ITA storica senza copertura D1 propria -- stessa famiglia del bug #8 di oggi, innescata dal dato sbagliato a monte invece che dall'assenza di logo in R2. Nessun esempio concreto trovato per verificarlo (richiederebbe interrogare l'API PokeWallet per set noti come solo-giapponesi, es. "Mega Evolution All-Stars"). Decisione dell'utente: chiudere la voce com'era scritta, non inseguire il caso limite senza un esempio reale -- da riprendere se emerge un logo sbagliato specifico riconducibile a questo meccanismo.
