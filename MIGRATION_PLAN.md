...
| 15 | Spezzare i file monolitici (`DeckLabScreen.kt` 2240 righe, `PokeTcgRepository.kt` 1735) | — **`DeckLabScreen.kt` rifatto 2026-09-09** (2291 -> 6 file, max 733 righe): il primo split della mattina era stato annullato dal merge (operava sulla versione precedente alla riscrittura DeckLab), rifatto sulla versione attuale e **verificato compilando**, con gli import potati per file. `CatalogRepository.kt` (ex `PokeTcgRepository.kt`) non e' lo stesso tipo di lavoro, vedi checkpoint |
