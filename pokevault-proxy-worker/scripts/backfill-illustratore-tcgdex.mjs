...
//
// Usage:
//   node scripts/backfill-illustratore-tcgdex.mjs <expansionId>              (dry-run: report only)
//   node scripts/backfill-illustratore-tcgdex.mjs <expansionId> --apply       (write to D1)
...
const tmpDir = path.join(__dirname, '.illustratore-tmp');
const CONCURRENCY = 8;
...
async function backfillExpansion(expansionId, apply) {
  ...
  if (!setSummary) {
    console.warn(`[${expansionId}] set TCGdex "${tcgdexId}" non trovato -- salto (${rows.length} carte senza illustratore)`);
  ...
  const results = await mapWithConcurrency(rows, CONCURRENCY, async (row) => {
    ...
    if (!ref) return { row, illustratore: null };
    const detail = await fetchJson(`https://api.tcgdex.net/v2/en/cards/${ref.id}`);
    return { row, illustratore: detail?.illustrator?.trim() || null };
  });

  const conIllustratore = results.filter((r) => r.illustratore);
  console.log(`[${expansionId}] ${conIllustratore.length}/${rows.length} carte con illustratore trovato (TCGdex: ${tcgdexId})`);

  if (apply && conIllustratore.length > 0) {
    await mkdir(tmpDir, { recursive: true });
    const statements = conIllustratore.map(
      // Solo dove manca: i set storici presi da Pokemon Central Wiki hanno
      // gia' l'illustratore letto dalla scheda italiana della carta, e non
      // c'e' motivo di sovrascriverlo con quello di TCGdex. Rende anche lo
      // script rilanciabile senza rimettere in discussione cio' che c'e' gia'.
      (r) => `UPDATE cards SET illustratore = ${sqlString(r.illustratore)} WHERE card_id = ${sqlString(r.row.card_id)} AND illustratore IS NULL;`
    );
  ...
  return { expansionId, matched: conIllustratore.length, total: rows.length };
}
...
async function main() {
  ...
  if (targets.length === 0) {
    throw new Error('Usage: node backfill-illustratore-tcgdex.mjs <expansionId> [--apply]  OR  --all [--apply]');
  }
  ...
  console.log('\n=== Riepilogo ===');
  console.log(`Carte totali: ${totalCards}, con illustratore: ${totalMatched} (${totalCards ? ((totalMatched / totalCards) * 100).toFixed(1) : 0}%)`);
  if (failed.length > 0) {
