...
// Solo per il confronto, pero': il numero che si SCRIVE non passa di qui se
// non e' numerico, o le varianti degli XY ("24a") finirebbero in catalogo come
...
// La grafia del set non si sceglie: si legge da com'e' scritto oggi.
function detectConvention(existingRows, setCodeUpper) {
  ...
  // ("24a"): li' si tiene la grafia di TCGdex tale e quale, che e' la stessa
  // dei set gia' scritti (verificato su smp e swshp prima del primo --apply).
...
async function detectR2Layout(setId, sampleNumber) {
...
async function main() {
  ...
  const layout = sampleOk ? await detectR2Layout(setId, String(sampleOk.card_number).trim()) : null;
  if (layout) {
