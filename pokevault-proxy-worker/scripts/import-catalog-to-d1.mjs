...
async function main() {
  ...
  const expansionRows = [...expansionCounts.entries()].map(
    ([id, count]) => `(${sqlString(id)}, ${count}, 100, NULL, 1)`
  );
  for (let i = 0; i < expansionRows.length; i += args.batchSize) {
    const chunk = expansionRows.slice(i, i + args.batchSize);
    lines.push(
      'INSERT INTO expansions (id, card_count, sort_order, logo_key, published) VALUES\n' +
      chunk.join(',\n') + '\nON CONFLICT(id) DO UPDATE SET card_count = excluded.card_count;'
    );
  }
