...
async function main() {
  ...
  const expansionRows = [...expansionCounts.entries()].map(([id, count]) => {
    const dominant = sqlString(dominantSetCode(expansionSetCodes.get(id) ?? []));
    // base_set_code and dominant_set_code hold the same value (schema/003 was added
    // twice under different names by two branches worked in parallel) -- written
    // together here so neither column goes stale on a re-import.
    return `(${sqlString(id)}, ${count}, 100, NULL, 1, ${dominant}, ${dominant})`;
  });
  for (let i = 0; i < expansionRows.length; i += args.batchSize) {
    const chunk = expansionRows.slice(i, i + args.batchSize);
    lines.push(
      'INSERT INTO expansions (id, card_count, sort_order, logo_key, published, dominant_set_code, base_set_code) VALUES\n' +
      chunk.join(',\n') +
      '\nON CONFLICT(id) DO UPDATE SET card_count = excluded.card_count, dominant_set_code = excluded.dominant_set_code, base_set_code = excluded.base_set_code;'
    );
  }
